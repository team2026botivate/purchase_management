...
export default function LiftReceiverPage() {
  ...
  const stageRecords = useMemo(() => {
    if (tabValue === 0) {
      return records.filter(r => r.workflowStage.liftReceiver === 'Pending');
    } else {
      return records.filter(r => r.workflowStage.liftReceiver === 'Completed');
    }
  }, [records, tabValue]);
  ...
  const handleCompleteProcess = (row) => {
    dispatch(completeStage({ id: row.id, currentStage: 'liftReceiver' }));
    toast.success(`Lift Receiver completed for Indent ${row.indentNumber} and moved to Tally Entry.`);
  };
  ...
  const actions = useCallback((row) => {
    if (tabValue === 0) {
      ...
      return [
        ...
        <PrintBtn key="print" onClick={() => printTable([row], WORKFLOW_COLUMNS.map((c) => ({ key: c.key, header: c.label })), `Lift Receiver for ${row.indentNumber}`)} />
      ];
    }
  }, [tabValue, dispatch]);
  ...
  return (
    <Box>
      ...
      <WorkflowTable 
        rows={filtered} 
        title={tabValue === 0 ? "Pending Lift Receiver" : "Lift Receiver History"} 
        actions={actions} 
      />
    </Box>
  );
