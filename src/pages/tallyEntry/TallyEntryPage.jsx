...
export default function TallyEntryPage() {
  ...
  const stageRecords = useMemo(() => {
    if (tabValue === 0) {
      return records.filter(r => r.workflowStage.tallyEntry === 'Pending');
    } else {
      return records.filter(r => r.workflowStage.tallyEntry === 'Completed');
    }
  }, [records, tabValue]);
  ...
  const handleCompleteProcess = (row) => {
    dispatch(completeStage({ id: row.id, currentStage: 'tallyEntry' }));
    toast.success(`Tally Entry completed for Indent ${row.indentNumber}. Process fully completed.`);
  };
  ...
  const actions = useCallback((row) => {
    if (tabValue === 0) {
      ...
      return [
        ...
        <PrintBtn key="print" onClick={() => printTable([row], WORKFLOW_COLUMNS.map((c) => ({ key: c.key, header: c.label })), `Tally Entry for ${row.indentNumber}`)} />
      ];
    }
  }, [tabValue, dispatch]);
  ...
  return (
    <Box>
      ...
      <WorkflowTable 
        rows={filtered} 
        title={tabValue === 0 ? "Pending Tally Entry" : "Tally Entry History"} 
        actions={actions} 
      />
    </Box>
  );
